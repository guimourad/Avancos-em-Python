...
- Algumas empresas definem que um cliente é considerado um cliente inadimplente
quando ele está devendo acima de X reais e por mais de X dias.
- Algumas empresas definem que um vendedor bateu a meta quando ele vendeu acima de X reais
(outras ainda analisam não só as vendas desse vendedor, 
mas também da loja ou da unidade de negócios que ele faz parte, ou ainda uma avaliação qualitativa)
- Algumas empresas definem que um produto está em "falta" no estoque
quando ele está abaixo de um nível mínimo quando um cliente insere um novo pedido daquele produto
- E assim vai para dezenas de indicadores.

O ponto importante é, cada empresa tem alguma adaptação do cálculo de indicadores.
E normalmente se formos analisar esses indicadores é interessante ter funções
que façam todo o trabalho de análise deles. Por isso, vamos construir 2 exemplos aqui:



### Item 1: Crie uma função que calcula o percentual de Stockout de uma empresa

- O % de stockout é dado por (Vendas Perdidas por Estoque) / (Vendas Concluídas + Vendas Perdidas por Estoque)
-> essas vendas são dadas em valor total (dinheiro) e não em quantidade de vendas
- Seu programa recebe um dicionário com todas as vendas da empresa,
o status dela (se foi Concluída ou Cancelada) e, caso tenha sido Cancelada,
o motivo de Cancelamento. O formato é o seguinte:
...
#- Para reforçar: As vendas Canceladas por qualquer outro motivo
# que não seja a Falta de Estoque não devem ser consideradas para a conta do Stockout.
...
def calculoStockout(dicionarioVendas):
    numerador = 0
    denominador = 0
    for venda in dicionarioVendas: 
        valor, status, motivo = dicionarioVendas[venda]
        if status == 'Concluído':
            denominador += valor
...
print(f'{calculoStockout(vendas):.2%}')

    
'''
for venda in vendas:
    valor, status, motivo = vendas[venda]
    print(valor, status, motivo)
'''
...
- O objetivo é identificar quem são os clientes inadimplentes
e enviar essa lista de clientes para o setor de cobrança poder fazer a cobrança dos clientes.
- Sua função deve então receber uma lista de clientes,
analisar quais clientes estão inadimplentes, e retornar uma lista com os clientes inadimplentes
(apenas o CPF deles já é suficiente)
- A inadimplência nessa empresa é calculada da seguinte forma:
    ...
    2. Isso significa que caso ou cliente esteja devendo 2.000 reais a 10 dias,
      ele não é inadimplente, pois não se passaram 20 dias ainda.
      Da mesma forma, se ele estiver devendo 500 reais por 40 dias,
      ele também não é inadimplente, dado que ele deve menos de 1.000 reais.
...
def clientesInadiplentes(ListaCliente):
    listaInadiplente = []
    for cliente in ListaCliente:
#        if cliente[1] > 1000 and cliente[2] > 20:
#            print(f'o cliente com o cpf {cliente[0]} esta inadiplente')
        cpf, valor, dias = cliente 
        if valor > 1000 and dias > 20:
            listaInadiplente.append(cpf)
    return listaInadiplente
#dessa maneira, fiz o unpacking da tupla
...
#clientesInadiplentes(clientes_devedores)

print(f'{clientesInadiplentes(clientes_devedores)}')