...
Muitas vezes as listas de produtos, de clientes, de funcionários, etc
que temos nas empresas não são 100% confiáveis.

Isso significa que na prática, caímos em muitos casos de ter uma lista de produtos,
mas quando olhamos por exemplo uma base com a posição em estoque de cada produto,
...
Vamos criar uma function que pega todas essas listas e cria, a partir dela,
 uma lista de produtos. Com o objetivo de saber todos os produtos que temos na empresa.
...
def listarProdutos(*listas):
    listaProdutos = []
    for lista in listas:
        for item in lista:
        #ou
        #for produto, estoque in lista:
            listaProdutos.append(item[0])
            #listaProdutos.append(produto)
    listaProdutos = set(listaProdutos) #-> transformando em um set para remover os itens iguais
    listaProdutos= list(listaProdutos) # -> transformando novamente em lista, com os itens iguais removidos
...
print(listarProdutos(estoque_fim_jan, estoque_fim_fev, estoque_fim_mar, estoque_fim_abr))
print(len(listarProdutos(estoque_fim_jan, estoque_fim_fev, estoque_fim_mar, estoque_fim_abr)))
