...
#- Vamos mudar a função que fizemos na aula passada para conseguir
# categorizar qualquer tipo de bebida de acordo com o "rótulo" passado para a nsosa function.
# Basicamente nossa function agora tem que verificar se o produto é da categoria passada ou não.
...
def ehDaCategoria(bebida, codigoCategoria):
    bebida = bebida.upper()
    if codigoCategoria in bebida:
        return True
    else:
        return False
...
for produto in produtos:
    if ehDaCategoria(produto, 'BEB'): #1. Em ordem (positional argument)

        print(f'enviar {produto} para setor de bebidas alcolicas')
    elif ehDaCategoria(codigoCategoria= 'BSA', bebida = produto): #2. Com o nome do argumento (keyword argument)
...
- Normalmente, nos nossos códigos, fazemos as definições de todas as funções antes e depois
construimos o resto do código.
